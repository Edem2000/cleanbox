...
@section('content')
        ...
        <div class="section-container">
                <div class="section-nav">
                  <div class="btn-container">
                    <div class=""></div>
                    <a href="{{ route('diplomas.create') }}" class="btn" id="create_news">Добавить сертификат</a>
                  </div>
                </div>
                @if(session('message'))
                  <div class="message">
                          ...
                          @foreach($diplomas as $diploma)
                              <div class="blog-card-container" data-slide-index="{{ $loop->iteration - 1 }}">
                                <div class="blog-card">
                                    ...
                                    <div class="card-info">
                                        <p class="name">{{ $diploma->__('name') }}</p>
                                        <div class="meta">
                                            <div class="meta-item">
                                                <img src="{{ asset('img/icons/calendar.svg') }}" alt="Date:" class="icon">
                                                <p class="text">{{ date("d-m-Y H:i", strtotime( $diploma->created_at)) }}</p>
                                            </div>
                                        </div>
                                      <div class="card-buttons">
                                        <a href="{{ route('diplomas.edit', $diploma) }}" class="btn blue">Редактировать</a>
                                        @if($diploma->active === 1 )
                                          <a href="{{ route('disableDiploma', $diploma) }}" class="btn grey">Скрыть</a>
                                        @else
                                          <a href="{{ route('enableDiploma', $diploma) }}" class="btn green">Показать</a>
                                        @endif
                                        <button class="btn red delete" data-article-id="{{ $diploma->id }}">
  ...
  <div class="custom-modal-overlay">
    <div class="custom-modal">
      <p class="heading">Вы действительно хотите удалить сертификат?</p>
      <form method="POST" action="{{ route('diplomas.destroy', 1) }}" class="buttons-block " id="admin_news_delete">
        @method('DELETE')
        @csrf
